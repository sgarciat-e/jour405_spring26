...
ANSWER HERE: The categories include both high-frequency incidents like accidents, theft, and auto theft, and lower-frequency violent crimes like homicide and assault shootings. Violent crimes carry more weight for public impact even though they happen less often, which makes the data harder to interpret—a handful of shootings and thousands of traffic accidents get treated as equivalent categories. The way incidents are split across multiple subcategories also makes it difficult to see patterns, since related crimes are separated rather than grouped together.
...
pg_crime_averages <- pg_crime %>%
  mutate(
    total_incidents = Jan + Feb + Mar + Apr + May + Jun +
...
ANSWER HERE
The data shows that high-volume categories—accident, auto stolen, theft from auto, and theft—dominate monthly incident counts, with several averaging hundreds of incidents per month. Meanwhile, homicide, assault shooting, assault weapon, and some robbery subcategories sit in the single digits or low teens. This gap reveals that most reported incidents in Prince George's County are property crimes and traffic-related events rather than serious violent crime, even though violent crime generates far more public concern. Robbery residential stands out as a problem because it has missing values, which makes it harder to spot trends and suggests there might be data reporting or classification issues that aren't immediately visible in other categories.
...
ANSWER HERE
Looking at the percentage changes, some categories show modest increases in June and July—especially auto-related crimes, assaults, and certain violent incidents—while others stay flat or 
even decline compared to their yearly averages. The pattern is mixed rather than showing a clear summer spike across the board. This suggests that while some types of crime do increase during summer months, the idea that all crime is more likely in summer is only partially true and depends on what type of incident you're looking at.
...
Lede: 
Property crimes and traffic-related incidents comprise the vast majority of reported crime in Prince George’s County, far outpacing violent offenses throughout the year.
Nutgraf: 
A recent analysis of county crime data reveals that high-volume incidents like theft, auto theft and traffic crashes happen at significantly higher rates than homicides or shootings. While 
specific crimes see minor summer increases, overall patterns remain driven by routine incidents rather than seasonal violence spikes. Officials said identifying these trends puts public 
safety concerns into context and directs law enforcement resources to areas where they may have the greatest impact.
